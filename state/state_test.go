...
func TestABCIResponsesSaveLoad1(t *testing.T) {
	...
	abciResponses.EndBlock = &abci.ResponseEndBlock{ValidatorUpdates: []abci.ValidatorUpdate{
		types.TM2PB.NewValidatorUpdate(signature.GenPrivKey().PubKey(), 10),
	}}
...
func genValSetWithPowers(powers []int64) *types.ValidatorSet {
	...
	for i := 0; i < size; i++ {
		...
		val := types.NewValidator(signature.GenPrivKey().PubKey(), powers[i])
		val.ProposerPriority = tmrand.Int64()
		vals[i] = val
	}
	valSet := types.NewValidatorSet(vals)
...
// see https://github.com/mihongtech/tendermint/issues/2718
func TestProposerPriorityDoesNotGetResetToZero(t *testing.T) {
	tearDown, _, state := setupTestCase(t)
	defer tearDown(t)
	val1VotingPower := int64(10)
	val1PubKey := signature.GenPrivKey().PubKey()
	...
	// add a validator
	val2PubKey := signature.GenPrivKey().PubKey()
...
func TestLargeGenesisValidator(t *testing.T) {
	...
	genesisPubKey := signature.GenPrivKey().PubKey()
	...
	// see: https://github.com/mihongtech/tendermint/issues/2960
	firstAddedValPubKey := signature.GenPrivKey().PubKey()
	...
	for i := 0; i < 10; i++ {
		addedPubKey := signature.GenPrivKey().PubKey()
